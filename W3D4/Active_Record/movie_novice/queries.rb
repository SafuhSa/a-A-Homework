...
def below_average_years
  ...
  Movie.select('yr', 'COUNT(*) as bad_movies').where('score < 5').order('bad_movies DESC').group('yr')
  # .group('yr AS bad_movies').select(:yr)
end
...
def pulp_fiction_actors
  ...
  Actor.joins(:movies).select(:name, :id).where("title LIKE 'Pulp Fiction'")
end
