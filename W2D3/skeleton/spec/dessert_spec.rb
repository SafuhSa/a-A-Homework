...
describe Dessert do
  let(:chef) { double("chef", name: "Safuh") }
  ...
  describe "initialize" do
    ...
    it "starts ingredients as an empty array" do
      expect(cake.ingredients.empty?).to be_truthy
    end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ...
      ingredients.each do |ingret|
        cake.add_ingredient(ingret)
      end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Safuh")
      expect(cake.serve).to eq("Chef Safuh has made 10 cakes!")
    end
